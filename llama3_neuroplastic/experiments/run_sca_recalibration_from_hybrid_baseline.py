...
class SCALocalRecalibrationConfig:
    ...
    sca_output_compensation_bias: bool = False
    ...
    ce_weight: float = 0.5
    decode_ce_weight: float = 1.0
    decode_teacher_kl_weight: float = 0.5
    decode_teacher_cos_weight: float = 0.25
    decode_local_alignment_weight: float = 0.05
    decode_latent_balance_weight: float = 0.1
    decode_block_balance_weight: float = 0.1
    decode_rollout_kl_weight: float = 1.0
    decode_rollout_cos_weight: float = 0.5
    ...
    dense_sparse_curriculum_enabled: bool = False
    dense_sparse_curriculum_warmup_steps: int = 8
    dense_sparse_curriculum_ramp_steps: int = 24
    block_banking_enabled: bool = False
    block_banking_low_usage_threshold: float = 0.001
    block_banking_vote_threshold: int = 3
    block_banking_cooldown_steps: int = 64
    block_banking_max_fraction: float = 0.25
    block_banking_usage_ema_decay: float = 0.95
...
def _parse_args() -> SCALocalRecalibrationConfig:
    ...
    p.add_argument("--freeze-basis-decoder", action="store_true")
    ...
    p.add_argument("--sca-output-compensation-bias", action=argparse.BooleanOptionalAction, default=False)
    ...
    p.add_argument("--decode-ce-weight", type=float, default=1.0)
    p.add_argument("--decode-teacher-kl-weight", type=float, default=0.5)
    p.add_argument("--decode-teacher-cos-weight", type=float, default=0.25)
    p.add_argument("--decode-local-alignment-weight", type=float, default=0.05)
    p.add_argument("--decode-latent-balance-weight", type=float, default=0.1)
    p.add_argument("--decode-block-balance-weight", type=float, default=0.1)
    p.add_argument("--decode-rollout-kl-weight", type=float, default=1.0)
    p.add_argument("--decode-rollout-cos-weight", type=float, default=0.5)
    ...
    p.add_argument("--dense-sparse-curriculum-enabled", action=argparse.BooleanOptionalAction, default=False)
    p.add_argument("--dense-sparse-curriculum-warmup-steps", type=int, default=8)
    p.add_argument("--dense-sparse-curriculum-ramp-steps", type=int, default=24)
    p.add_argument("--block-banking-enabled", action=argparse.BooleanOptionalAction, default=False)
    p.add_argument("--block-banking-low-usage-threshold", type=float, default=0.001)
    p.add_argument("--block-banking-vote-threshold", type=int, default=3)
    p.add_argument("--block-banking-cooldown-steps", type=int, default=64)
    p.add_argument("--block-banking-max-fraction", type=float, default=0.25)
    p.add_argument("--block-banking-usage-ema-decay", type=float, default=0.95)
...
def _compute_dense_sparse_curriculum_alpha(step: int, cfg: SCALocalRecalibrationConfig) -> float:
    if not bool(cfg.dense_sparse_curriculum_enabled):
        return 1.0
    warmup = int(max(0, cfg.dense_sparse_curriculum_warmup_steps))
    ramp = int(max(0, cfg.dense_sparse_curriculum_ramp_steps))
    if int(step) < warmup:
        return 0.0
    if ramp <= 0:
        return 1.0
    progress = float(int(step) - warmup + 1) / float(ramp)
...
def main() -> None:
    ...
    print(f"DEBUG: Loading hybrid artifact from {cfg.hybrid_checkpoint}")
    artifact = _load_hybrid_artifact(cfg.hybrid_checkpoint)
    print("DEBUG: Loaded artifact")
    ...
    model = NeuroplasticLlama(
        ...
        sca_dense_anchor_stride=int(cfg.sca_dense_anchor_stride),
        ...
        sca_output_compensation_bias_enabled=bool(cfg.sca_output_compensation_bias),
    ...
    trainable = model.prepare_sca_local_recalibration(
        ...
        freeze_basis_decoder=bool(cfg.freeze_basis_decoder),
    ...
    model.configure_sparse_block_banking(
        ...
        low_usage_threshold=float(cfg.block_banking_low_usage_threshold),
        vote_threshold=int(cfg.block_banking_vote_threshold),
        cooldown_steps=int(cfg.block_banking_cooldown_steps),
        max_fraction=float(cfg.block_banking_max_fraction),
        usage_ema_decay=float(cfg.block_banking_usage_ema_decay),
    ...
    for step in range(total_steps):
        ...
        curriculum_alpha = _compute_dense_sparse_curriculum_alpha(step=step, cfg=cfg)
        model.set_sparse_curriculum(
            enabled=bool(cfg.dense_sparse_curriculum_enabled),
            alpha=float(curriculum_alpha),
        ...
        aux["curriculum_alpha"] = float(curriculum_alpha)

        if decode_manifold_mode:
            use_teacher_kl = abs(float(cfg.decode_teacher_kl_weight)) > 0.0
            use_teacher_cos = abs(float(cfg.decode_teacher_cos_weight)) > 0.0
            use_rollout_alignment = stage_idx >= 1 and (
                abs(float(cfg.decode_rollout_kl_weight)) > 0.0
                or abs(float(cfg.decode_rollout_cos_weight)) > 0.0
            )
            dense_out = None
            ...
            teacher_cos = torch.zeros((), device=model.device, dtype=torch.float32)
            if use_teacher_cos and dense_out is not None:
            ...
            total_loss = (
                (float(cfg.decode_ce_weight) * ce_loss)
                + (float(cfg.decode_teacher_kl_weight) * teacher_kl)
                + (float(cfg.decode_teacher_cos_weight) * teacher_cos)
                + (float(cfg.decode_local_alignment_weight) * local_loss)
                + (float(cfg.decode_latent_balance_weight) * latent_balance)
                + (float(cfg.decode_block_balance_weight) * block_balance)
            )
            ...
            if use_rollout_alignment:
                ...
                total_loss = (
                    total_loss
                    + (float(cfg.decode_rollout_kl_weight) * rollout_kl)
                    + (float(cfg.decode_rollout_cos_weight) * rollout_cos)
                )
    ...
    metrics = {
        ...
        "final_curriculum_alpha": float(_compute_dense_sparse_curriculum_alpha(step=max(total_steps - 1, 0), cfg=cfg)),
        "sparse_block_banking": model.get_sparse_block_banking_stats(),
        "config": asdict(cfg),
        "stage_plan": stage_plan,
    }

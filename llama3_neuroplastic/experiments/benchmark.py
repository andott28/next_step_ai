...
def run_benchmark(
    ...
    *,
    ...
    if getattr(runtime, "_vram_hot_cache_enabled", False) and getattr(runtime, "_sparse_routing", None):
        # Calibrate from the actual timed prompt (not warmup-generated tokens) so that
        # MLP hot-block selection is based on the same positional context as the decode.
        # Using warmup_generated tokens at re-indexed positions 0-15 would mispredict
        # decode routes at position prompt_len+.
        _calib_ids = input_ids.detach().to(device=torch.device("cpu"), dtype=torch.long)

...
def _extract_recal_summary(path: Path) -> Dict[str, Any]:
    ...
    return {
        ...
        "quality_gate_failure_reason": data.get("quality_gate_failure_reason", ""),
        ...
        "max_latent_importance_fraction": balance.get("max_latent_importance_fraction"),
        "max_block_importance_fraction": balance.get("max_block_importance_fraction"),
        "max_latent_load_fraction": balance.get("max_latent_load_fraction"),
...
def _extract_probe_summary(path: Path) -> Dict[str, Any]:
    ...
    for row in data.get("rows", []):
        prompt_rows = row.get("prompts", [])
        text = prompt_rows[0].get("text", "") if prompt_rows else ""
        rows.append(
            {
                "decode_guard_layers": row.get("decode_guard_layers"),
                "mean_deg": row.get("mean_deg"),
                "mean_rep": row.get("mean_rep"),
                "mean_touched_weight_fraction": row.get("mean_touched_weight_fraction"),
                "text": text,
            }
        )
    return {"rows": rows}
...
def _build_recal_cmd(args: argparse.Namespace, output_dir: Path, progressive: bool) -> List[str]:
    cmd = [
        sys.executable,
    ...
    if progressive:
        ...
        cmd.append("--no-progressive-depth-enabled")
    return cmd
...
def _run_arm(args: argparse.Namespace, workdir: Path, output_root: Path, name: str, progressive: bool) -> Dict[str, Any]:
    arm_dir = output_root / name
    arm_dir.mkdir(parents=True, exist_ok=True)
    recal_cmd = _build_recal_cmd(args, arm_dir, progressive=progressive)
    _run(recal_cmd, workdir)

    recal_metrics = arm_dir / "sca_recalibration_metrics.json"
    recal_state = arm_dir / "sca_recalibrated_state.pt"
    probe_json = arm_dir / "fast_depth_probe.json"
    probe_cmd = _build_probe_cmd(args, recal_state, probe_json)
    _run(probe_cmd, workdir)
...
def main() -> None:
    p = argparse.ArgumentParser(description="Fast smoke A/B for one-shot vs progressive sparse recalibration.")
    ...
    p.add_argument("--layers", type=str, default="2-7")
    p.add_argument("--sca-bottom-buffer-layers", type=int, default=2)
    p.add_argument("--sca-decode-guard-layers", type=int, default=24)
    ...
    p.add_argument("--validation-prefix-count", type=int, default=4)
    ...
    p.add_argument("--probe-decode-guards", type=str, default="26,24")
    ...
    summary = {
        "config": vars(args),
        "runs": {
            "one_shot": _run_arm(args, workdir, output_root, "one_shot", progressive=False),
            "progressive": _run_arm(args, workdir, output_root, "progressive", progressive=True),
        },
    }

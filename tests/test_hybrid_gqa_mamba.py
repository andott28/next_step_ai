...
from run_sca_recalibration_from_hybrid_baseline import (
    SCALocalRecalibrationConfig,
    _compute_latent_support_regularizer,
)
...
def test_hybrid_attention_state_loads_through_tier_mask_wrapper(tmp_path):
    model_a = _build_fake_neuroplastic_model([_Layer(_hybrid(1.0, -1.0, mix_init=0.2))])
    exported = NeuroplasticLlama.export_hybrid_attention_state(model_a)
    ckpt = tmp_path / "hybrid_wrapped.pt"
    torch.save(exported, ckpt)

    wrapped_attn = _TierMaskedLlamaAttention(_hybrid(2.0, 3.0, mix_init=0.8), lambda: None)
    model_b = _build_fake_neuroplastic_model([_Layer(wrapped_attn)])
    info = NeuroplasticLlama.load_hybrid_attention_state(model_b, str(ckpt), strict=True)

    assert info == {"loaded_layers": 1, "missing_layers": 0}
    layer_attn = model_b.model.model.layers[0].self_attn.base_attn
    assert abs(float(layer_attn.mix_value.item()) - float(model_a.model.model.layers[0].self_attn.mix_value.item())) < 1e-5
...
def test_export_sca_recalibration_state_round_trip_with_learned_basis_wrapper(tmp_path):
    ...
    assert model_b.sca_config.output_compensation_bias_enabled is True
...
def test_inference_loader_accepts_decoder_mirror_checkpoint():
    ...
    assert args.sca_dense_anchor_stride == 6
    assert args.strict_decode_upper_layer_cap_enabled is False


def test_sparse_layer_enabled_respects_dense_anchor_stride_relative_to_active_band():
    model = object.__new__(NeuroplasticLlama)
    nn.Module.__init__(model)
    model.neuroplasticity_enabled = True
    model.model = types.SimpleNamespace(model=types.SimpleNamespace(layers=[object()] * 32))
    model.bottom_buffer_layers = 3
    model.buffer_layers = 0
    model.decode_guard_layers = 0
    model.sca_dense_anchor_stride = 6
    model._sca_sparse_layer_override = None

    active = [idx for idx in range(32) if NeuroplasticLlama._sparse_layer_enabled(model, idx)]

    assert 8 not in active
    assert 14 not in active
    assert 20 not in active
    assert 26 not in active
    assert 3 in active
    assert 7 in active
    assert 9 in active


def test_recalibration_config_accepts_dense_anchor_stride():
    cfg = SCALocalRecalibrationConfig(
        model_name="toy-model",
        hybrid_checkpoint="hybrid.pt",
        output_dir="out",
        sca_dense_anchor_stride=6,
    )
    assert cfg.sca_dense_anchor_stride == 6
